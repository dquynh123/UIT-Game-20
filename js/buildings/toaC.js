import { handleGameEnding } from '../ending.js';
...
function startToaCGame() {
    ...
    if (typeof uitQuizBank !== 'undefined') {
        ...
        console.error("❌ Lỗi: Không tìm thấy data câu hỏi (uitQuizBank).");
    }
}

// Cảm biến lắng nghe khi mở Tòa C
...
// HÀM ENDGAME - CHUYỂN GIAO CHO ENDING.JS
function endGame() {
    ...
    const proceedToNextPhase = () => {
        ...
        // GỌI FILE ENDING.JS
        handleGameEnding(finalTotalScore, isWin, playerName, totalTime, results);
    }; 

    const isWinStage = (wrongCount < 5); 
    if (typeof window.showGlobalSummaryBoard === 'function') {
        window.showGlobalSummaryBoard("Tòa C", score, 0, isWinStage, proceedToNextPhase);
    } else {
        proceedToNextPhase();
    }
}
...
// ==========================================
setTimeout(() => {
    if (toaCContainer && toaCContainer.style.display === 'block' && gameState === "MENU") {
        startToaCGame();
    }
}, 500);
